#import "LAColorPickerCell.h"

@implementation LAColorPickerCell
...
- (void)openColorPicker {
  ...
  if (@available(iOS 14.0, *)) {
    ...
    colourPickerVC.selectedColor = [self colorWithHexString:[[LAPrefs shared] objectForKey:self.specifier.properties[@"key"] defaultValue:self.specifier.properties[@"fallback"] ID:self.specifier.properties[@"defaults"]]];
...
- (void)updatePreview {
  ...
  self.colorPreview.backgroundColor = [self colorWithHexString:[[LAPrefs shared] objectForKey:self.specifier.properties[@"key"] defaultValue:self.specifier.properties[@"fallback"] ID:self.specifier.properties[@"defaults"]]];
...
- (void)colorPickerViewControllerDidSelectColor:(UIColorPickerViewController *)viewController API_AVAILABLE(ios(14.0)){
  ...
  [[LAPrefs shared] setObject:[self hexStringFromColor:selectedColour] forKey:self.specifier.properties[@"key"] ID:self.specifier.properties[@"defaults"]];
  [self updatePreview];
}
